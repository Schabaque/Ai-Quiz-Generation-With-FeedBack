...
const TitleSection = (props) => {
  ...
  const handleSubmit = (event) => {
    ...
    if (titre.trim() === "") return toast.error("Title is required");
    if (description.trim() === "") return toast.error("Add a description");
    if (!image) return toast.error("Add an image for this class");
          ...
          <div className="modal-content">
            <form onSubmit={handleSubmit}>
              <div className="modal-header">
                <h5 className="modal-title" id="add-modal-label">Add New Class</h5>
                <button type="button" id="colseModal" className="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
              </div>
              <div className="modal-body">
                <div className="mb-3">
                  <label htmlFor="item-name" className="form-label">Title</label>
                  <input
                    type="text"
