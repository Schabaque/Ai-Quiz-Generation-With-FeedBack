...
const ExamSubmition = (props) => {
  ...
  const columns = [
    {
      name: 'photo',
      selector: (row) => row.student?.photo?.url || defaultAvatar,
      cell: (row) => <img style={{ width: '40px', height: '40px' }} src={row.student?.photo?.url || defaultAvatar} alt={row.student.firstname} />,
    },
    {
      name: 'Nom  ',
